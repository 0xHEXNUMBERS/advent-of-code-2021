...
func (s *SnailfishNumber) Copy() *SnailfishNumber {
	...
	out := &SnailfishNumber{}
	out.Left = s.Left.Copy()
	out.Right = s.Right.Copy()
	out.Number = s.Number
	return out
}

func (s *SnailfishNumber) Add(s2 *SnailfishNumber) *SnailfishNumber {
	sSum := &SnailfishNumber{Left: s.Copy(), Right: s2.Copy()}
	sSum.Reduce()
	return sSum
}
...
func main() {
	...
	numberStrings := strings.Split(string(input), "\n")
	numbers := make([]*SnailfishNumber, len(numberStrings))
	for i := 0; i < len(numbers); i++ {
		numbers[i] = parseSnailfishNumber(numberStrings[i])
	}
	...
	for i := 0; i < len(numbers); i++ {
		for j := 0; j < len(numbers); j++ {
			...
			mag := numbers[i].Add(numbers[j]).Magnitude()
			if maxMag < mag {
				maxMag = mag
			}
		}
